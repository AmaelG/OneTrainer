        ...
        if self.__needs_validate(train_progress):
            ...
            for validation_batch in step_tqdm_validation:
                ...
                for task, loss in losses.items():
                    ...
                    mapping_key = (concept_seed, task)
                    # check and fix collision to display both graphs in tensorboard
                    if label in mapping_label_to_seed_task and mapping_label_to_seed_task[label] != mapping_key:
                        suffix = 1
                        new_label = f"{label}({suffix})"
                        while new_label in mapping_label_to_seed_task and mapping_label_to_seed_task[new_label] != mapping_key:
                            suffix += 1
                            new_label = f"{label}({suffix})"
                        label = new_label

                    if mapping_key not in mapping_seed_task_to_label:
                        mapping_seed_task_to_label[mapping_key] = label
                        mapping_label_to_seed_task[label] = mapping_key

                    accumulated_loss_per_concept_task[mapping_key] = \
                        accumulated_loss_per_concept_task.get(mapping_key, 0) + loss
                    concept_task_counts[mapping_key] = concept_task_counts.get(mapping_key, 0) + 1

            for mapping_key, total_loss in accumulated_loss_per_concept_task.items():
                average_loss = total_loss / concept_task_counts[mapping_key]

                self.tensorboard.add_scalar(f"loss/validation_step/{mapping_seed_task_to_label[mapping_key]}",
                                            average_loss,
                                            train_progress.global_step)
                        ...
                        if loss_value > self.high_loss_threshold and multi.is_master():
                            print(
                                "HIGH LOSS",
                                {
                                    ...
                                    "raw_pixel_image_stats": self.__debug_tensor_stats(batch.get("pixel_image")),
                                    "pixel_image_stats": self.__debug_tensor_stats(model_output_data.get("pixel_image")),
