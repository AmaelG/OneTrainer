...
class NanoSaurTransformer2DModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.mask_token2 = nn.Parameter(torch.zeros(1, 1, hidden_size, device=device))
    ...
    def _forward(self, x: torch.Tensor, timesteps: torch.Tensor, context: torch.Tensor, uncond: bool = False, token_weights: torch.Tensor | None = None) -> torch.Tensor:
        ...
        x_tokens = x.reshape(batch, self.in_channels, h_patches, self.patch_size, w_patches, self.patch_size)
        x_tokens = x_tokens.permute(0, 2, 4, 1, 3, 5).reshape(batch, h_patches * w_patches, self.in_channels * self.patch_size**2)
        ...
        x_dec = x_dec.reshape(batch, h_patches, w_patches, self.in_channels, self.patch_size, self.patch_size)
        return x_dec.permute(0, 3, 1, 4, 2, 5).reshape(batch, self.in_channels, height, width)
