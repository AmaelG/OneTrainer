...
class BaseTrainUIView(ABC):
    ...
    def build_general_tab_content(self, frame, controller, ui_state):
        ...
        self.components.label(frame, 8, 2, "Validation Timesteps",
                         tooltip="Comma-separated list of timesteps used for validation, between 0 and 999. High values determine composition, low values details.")
        self.components.entry(frame, 8, 3, ui_state, "validation_timesteps")

        self.components.label(frame, 9, 0, "Validate after",
    ...
    def build_data_tab_content(self, frame, controller, ui_state):
        ...
        self.components.entry(frame, 3, 1, ui_state, "caching_threads", required=True)
