...
class ModelType(Enum):
    ...
    def supported_training_methods(self) -> tuple[TrainingMethod, ...]:
        ...
        if self.is_qwen() or self.is_anima() or self.is_z_image() or self.is_flux_2() or self.is_ernie() or self.is_lens() or self.is_ideogram():
    ...
    def supported_lora_formats(self) -> list[ModelFormat]:
        ...
        if not (self.is_hi_dream() or self.is_sana() or self.is_wuerstchen_v2() or self.is_anima() or self.is_lens() or self.is_ideogram()):
...
_MODEL_PARTS: dict[ModelType, tuple[str, ...]] = {
    ...
    ModelType.LENS: ("transformer", "text_encoder", "vae"),
