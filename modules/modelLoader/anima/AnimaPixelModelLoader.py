...
class AnimaPixelModelLoader(HFModelLoaderMixin):
    ...
    @staticmethod
    def __pixel_transformer_from_base(base_transformer: AnimaTransformer3DModel) -> AnimaTransformer3DModel:
        config = base_transformer.config
        pixel_transformer = AnimaTransformer3DModel(
            in_channels=3,
            out_channels=3,
            ...
            adapter_vocab_size=config.adapter_vocab_size,
            adapter_dim=config.adapter_dim,
            adapter_layers=config.adapter_layers,
            adapter_heads=config.adapter_heads,
            adapter_sequence_length=config.adapter_sequence_length,
        )
        ...
        print(f"ANIMA_PIXEL on-load conversion copied {copied} tensors, initialized {skipped} pixel tensors")
    ...
    def __load_diffusers(
        ...
        self._prepare_sub_modules(
            base_model_name,
            diffusers_modules=["transformer"],
            transformers_modules=["text_encoder"],
        )

        qwen_tokenizer = AutoTokenizer.from_pretrained(base_model_name, subfolder="tokenizer")
        t5_tokenizer = AutoTokenizer.from_pretrained(base_model_name, subfolder="tokenizer_2")
        ...
        detailer_head = AnimaPixelDetailerHead(feature_channels=transformer.config.num_attention_heads * transformer.config.attention_head_dim)
