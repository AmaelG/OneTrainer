...
class AnimaPixelSampler(BaseModelSampler):
    ...
    def __sample(self, sample_config: SampleConfig, on_update_progress: Callable[[int, int], None]) -> ModelSamplerOutput:
        ...
        self.model.detailer_head_to(self.train_device)
        self.model.eval()
        ...
        num_train_timesteps = float(noise_scheduler.config.num_train_timesteps)

        with self.model.autocast_context:
            batch_size = 2 if sample_config.cfg_scale > 1.0 else 1
            text_encoder_output = self.model.encode_text(
                train_device=self.train_device,
                text=[sample_config.prompt, sample_config.negative_prompt or ""] if batch_size == 2 else sample_config.prompt,
            )

            self.model.text_encoder_to(self.temp_device)
            torch_gc()

            image = torch.randn((1, 3, height, width), generator=generator, device=self.train_device, dtype=torch.float32)
            timesteps = noise_scheduler.timesteps
            for i, timestep in enumerate(tqdm(timesteps, desc="sampling")):
                image_model_input = torch.cat([image] * batch_size)
                timestep_batch = timestep.expand(batch_size).to(device=self.train_device, dtype=torch.float32)
                model_timestep = timestep_batch / num_train_timesteps
                flow = self.model.predict_pixel_flow(
                    noisy_image=image_model_input.to(dtype=self.model.train_dtype.torch_dtype()),
                    timestep=model_timestep,
                    text_encoder_output=text_encoder_output,
                )
                if batch_size == 2:
                    flow_text, flow_uncond = flow.chunk(2)
                    flow = flow_uncond + sample_config.cfg_scale * (flow_text - flow_uncond)

                image = noise_scheduler.step(flow.float(), timestep, image, return_dict=False)[0]
                on_update_progress(i + 1, sample_config.diffusion_steps)

        sampler_output = ModelSamplerOutput(file_type=FileType.IMAGE, data=self.__tensor_to_image(image))
