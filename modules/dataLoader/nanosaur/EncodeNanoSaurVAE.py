...
class EncodeNanoSaurVAE(PipelineModule, RandomAccessPipelineModule):
    ...
    def get_item(self, variation: int, index: int, requested_name: str = None) -> dict:
        ...
        with torch.no_grad(), self._all_contexts(self.autocast_contexts):
            latent = self.model.encode_vae(image.unsqueeze(0)).squeeze(0)
        return {self.out_name: latent.detach().cpu()}
